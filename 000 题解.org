...
#+END_SRC
*** DONE hdu-1043 [[file:hdu1043.cpp][A* 904ms]]  [[file:hdu1043_rbfs.cpp][逆向bfs 187ms]]    八数码
CLOSED: [2017-09-17 周日 00:18]
**** 思路
1. A*，所有数字到目标位置的最短距离之和做为估价函数h，不包括空格，
   h一定是小于实际需要的步数的，所以能找到最优解
   g是已经走的步数，g+h单调不减，
   一个状态出队时，它的g+s就是最小值，后面不会出现该状态更小的g+s值

2. 由于目标状态只有一个，所以可以逆向广搜，然后打表
   类似的一道图论题：有向图求单源来回的最短路径

3. 双向广搜，
   多个case里，从目标状态出发的广搜不清空，继续上个case搜并打表
